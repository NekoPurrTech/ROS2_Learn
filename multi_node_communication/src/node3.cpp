...
public:
    Node3() : Node("node3") {
        // 创建发布者，向node1和node2发送消息
        publisher_to_node1_ = this->create_publisher<std_msgs::msg::String>("node3_to_node1", 10);
        publisher_to_node2_ = this->create_publisher<std_msgs::msg::String>("node3_to_node2", 10);

        // 创建订阅者，接收来自node1和node2的消息
        subscription_from_node1_ = this->create_subscription<std_msgs::msg::String>(
            "node1_to_node3", 10, std::bind(&Node3::callback_from_node1, this, std::placeholders::_1));
        subscription_from_node2_ = this->create_subscription<std_msgs::msg::String>(
            "node2_to_node3", 10, std::bind(&Node3::callback_from_node2, this, std::placeholders::_1));

        // 创建定时器，每秒发送一次消息
        timer_ = this->create_wall_timer(
            std::chrono::seconds(1), std::bind(&Node3::timer_callback, this));
    }

private:
    void timer_callback() {
        std_msgs::msg::String msg;
        msg.data = "Hello from Node3";
